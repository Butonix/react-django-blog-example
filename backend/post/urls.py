...
urlpatterns = [
	...
	url(r"^(?P<pk>[0-9]{1,6})/comments/$",comment_views.CommentList.as_view() , name= "comment-list"),
	url(r"^(?P<pk>[0-9]{1,6})/comments/(?P<comment_pk>[0-9]+)/$",comment_views.CommentDetail.as_view(), name='comment-detail'),
	url(r"^(?P<pk>[0-9]{1,6})/comments/(?P<comment_pk>[0-9]+)/commentreplies/$",comment_views.CommentReplyList.as_view(), name='commentreply-list'),
